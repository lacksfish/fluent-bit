...
void flb_test_filter_parser_handle_time_key_with_timezone_1()
{
    int ret;
    int bytes;
    char *p, *output, *expected;
    ...
    /* Parser */
    parser = flb_parser_create("timestamp", "regex", "^(?<time>.*)$", "%Y-%m-%dT%H:%M:%S,%L%z",
                               "time",
                               NULL, MK_FALSE,
                               NULL, 0, NULL, ctx->config);
    TEST_CHECK(parser != NULL);
...
void flb_test_filter_parser_handle_time_key_with_timezone_2()
{
    int ret;
    int bytes;
    char *p, *output, *expected;
...
void flb_test_filter_parser_handle_time_key_with_timezone_should_fail()
{
    int ret;
    int bytes;
    char *p, *output, *expected;
